...
make_table <- function(fit, caption=NULL){
  fit_sum <- summary(fit)
  ...
  kableExtra::kbl(tbl_sum, digits = 2, booktabs = T, caption=caption) %>% 
    kableExtra::kable_classic(full_width=F)
}

make_table(fit_s2_vaccIntent, "Bayesian regression model coefficients for model of Vaccine Intentions in Experiment 2.")
...
s2_diff_plot <- s2_scored %>% 
  ...
  labs(title="A", y = "Mean difference", x = "Belief scale", color="Intervention")
...
s2_diff_plot + s2_obs_pred_plot + plot_layout(ncol=2, widths = c(2,1))
```
...
make_table(fit_s3_vaccIntent, "Bayesian regression model coefficients for model of Vaccine Intentions in Experiment 3.")
```
...
s3_diff_plot + s3_obs_pred_plot + plot_layout(ncol=2, widths = c(2,1))
```
...
make_table(fit_s4_vaccIntent, "Bayesian regression model coefficients for model of Vaccine Intentions in Study 4.")
```
...
s4_diff_plot <- combined %>% 
  ...
  ggplot(aes(x=reorder(scale, mean), y = mean, ymin =ll, ymax=ul, color=vacc_news)) +
  geom_point(position=position_dodge(width=.25), size=2, shape=16) +
  geom_errorbar(position=position_dodge(width=.25), width=0, alpha=.8) +
  # geom_errorbar(aes(ymin=ll95, ymax = ul95), width=0, position=position_dodge(width=.25), alpha=.5) +
  geom_hline(yintercept = 0, linetype="dashed",alpha=.5) +
  scale_color_manual(values=c("#0067b1","#c50024")) +
  theme_bw(base_size=11) +
  ...
  labs(title="A", y = "Mean difference", x = "Belief scale", color="News Exposure")
...
s4_diff_plot + s4_obs_pred_plot + plot_layout(ncol=2, widths = c(2,1))
```
