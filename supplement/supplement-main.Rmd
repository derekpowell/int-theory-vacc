...
```{r dag_example, echo=F, fig.align='center', out.width = '40%', fig.cap="Example DAG illustrating some variables related to wildfires."}
...
if (!file.exists("../local/cv-res.rds")){
...
We used a bootstrapping procedure to estimate the confidence in each of the links in this model, conducting the structure learning procedure repeatedly for 200 bootstrapped resamples of the original training data. Figure X visualizes the frequency with which each arc appeared in the resulting model across these replicates, and figure x visualizes the model with the coefficients for each link encoded by color. 
...
We can examine the robustness of the model’s features in two ways. First, we can ask about the “skeleton” of the graphical model: whether the relationships between pairs of nodes are robust, irrespective of their directions. To test this, we applied our chosen structure learning algorithm against 1000 bootstrapped replicates of our data (following Friedman et al., 2013). As shown in figure X, we have a high degree of confidence in the “skeleton” of the graph: nodes which are connected in the final graph are reliably connected across bootstrapping runs. 
...
if (!file.exists("../local/boot_res.rds")){
  ...
  boot_res <- readRDS("../local/boot_res.rds")
}
...
```{r boot_strength_plot, echo=F, message=F, fig.cap="Bootstrapped confidence estimates for Bayesian network edges. Edge color indicates frequency with which edges appeared when conducting search across X bootstrap replicates. Only edges appearing in at least 20\\% of graphs are shown."}
# 
# hyde_str <- df_to_hyde_string(boot_res %>% filter(strength>.2, direction>0) %>% select(from,to))
# h2 <- HydeNetwork(as.formula(hyde_str))
# # plot(h2)
# 
# z2 <- custom_plot.HydeNetwork(h2)
# boot_res %>%
...
# # z2$nodes_df$fillcolor <- ifelse(z2$nodes_df$id==13, "grey","white")
# z2$nodes_df$fixedsize <- FALSE
# z2$edges_df$penwidth <- abs(edge_df_mod$strength)*2
# z2$edges_df$penwidth <- 2
...
Next, we can ask about the directions of the relationships within the model by examining the “completed partially directed acyclic graph” (CPDAG) derived from the structure learning algorithm. This is a graph with both directed and undirected edges which represents the “Markov equivalence class” of DAGs: the set of DAGs representing equivalent patterns of conditional (in)dependence among variables. Alternatively, this can be conceived of as “score equivalence,” the set of graphs which would produce equivalent scores for the observed data (Chickering, 2013). The CPDAG derived from our fit model contains 14 directed edges and 15 undirected arcs—indicating that the data do not constrain the directions of 15 of the edges in the final model. Of these undirected arcs, 10 directions are determined by our generative model constraints that we applied before fitting the model. Five arcs were determined neither by these constraints nor the data: these were $\text{holistic balance} \leftrightarrow \text{naturalism}$; $\text{medical skepticism} \leftrightarrow  \text{parental expertise}$, $\text{vaccine toxicity} \leftrightarrow \text{vaccine efficacy}$, $\text{vaccine toxicity} \leftrightarrow \text{vaccine danger}$ and $\text{vaccine danger} \leftrightarrow \text{vaccine efficacy}$. Thus, we feel that no strong conclusions should be drawn surrounding the counterintuitive relationships regarding vaccine toxicity, danger, and effectiveness. 
...
# do plotting -- will likely see lots of ambiguity??
plot(main_cpdag)

# CPDAG edge directions ruled out by blacklist (10)
...
```{r main_model_plot, echo=F, fig.width=8, fig.height=5, fig.align='center', fig.cap="Bayesian network model of intuitive theory surrounding vaccination decisions. Edge color indicates the magnitude of the generative (blue) or preventive (red) coeffient for the links."}
...
z$nodes_df$fillcolor <- ifelse(z$nodes_df$id==11, "grey","white")
...
# export graph image and render
export_graph(z, "../local/main-graph.pdf")
render_graph(z)
```
...
plt.valid
```
For each node, we also compared the Bayesian network model’s predictions to a naive “maximal” model using values on all other belief scales as predictors. Our intention was to capture the degree to which each belief is predictable from the others as a whole. As shown in table X, the relatively simpler Bayesian network model does not sacrifice any predictive power compared to the suite of maximal naive models.
...
To do this, we drew on a variety of sources, including a qualitative survey with participants in a previous study who had self-identified as vaccine skeptics. Out of 23 people invited to participate, n=16 participants completed the survey via Amazon Mechanical Turk (MTurk). All participants answered “yes” to the question “Are you concerned that childhood vaccinations might cause serious side-effects?” When asked, “Do you have concerns about all vaccines or only some specific vaccines?” nearly all participants (14 of 16) expressed being concerned about specific vaccines; 3 of these participants, as well as the remaining 2 participants, expressed concern about all vaccines.
...
```{r s2_model, echo=FALSE, message=F, warning=F}
...
make_table <- function(table_df, caption=NULL){
  ...
  kableExtra::kbl(table_df, digits = 2, booktabs = T, caption=caption, escape = T, format = "latex") %>% 
...
fit_s2_vaccIntent %>% 
  ...
  mutate(
    Term = case_when(
      # grepl("(diseaseRisk|post)", Term) ~ paste0("$\\beta{", 
...
To model the changes in the belief network, we estimate the “evidence ratio” using the method described in the modeling section above. The maximum likelihood estimate of the evidence ratio was XXX. To simulate the impacts of this evidence, we supplied virtual evidence to the disease severity node by adding a child node to the cognitive-model network with a CPT capturing this evidence ratio. Given that our cognitive model is parameterized via noisy-logical functions, the predicted influence of each belief is closely related to its first-order correlation.

Figure X (right) shows the correlation between predicted and observed belief changes conditional on the disease risk intervention.
...
s2_diff_plot <- s2_scored %>% 
  ...
  ggplot(aes(x = reorder(scale,M), y = M, ymin = M-se*2, ymax=M+se*2, color=condition, shape=condition)) +
  ...
  labs(title="A", y = "Mean difference", x = "Belief scale", color="Intervention", shape="Intervention")
...
```
Guided by these predictions, we identified beliefs about toxins in vaccines as a potentially useful site of intervention--it is among the most strongly related and it is a concrete belief that could be addressed directly. We developed an educational intervention aimed at alleviating these concerns and tested its efficacy in two experiments.
...
Procedures for studies 3a and 3b closely followed those of Study 2, but these studies tested the effects of a novel intervention aimed at dispelling concerns about toxic additives in vaccines. Full text of this intervention is available in appendix C of these supplementary materials. In both studies, participants were randomly assigned to either a no-intervention control condition or to the novel vaccine toxins intervention.

### Results. 

Due to the similarity of their designs, we combined data from studies 3a and 3b for our primary analysis. However, for full transparency we first describe analyses for these studies individually. As in Experiment 2, we conducted a mixed-effects Bayesian ordinal regression to assess the effects of the vaccine toxins intervention on participants’ _Vaccination Intentions_ scores. The vaccine toxins intervention improved participant’s vaccination intention scores relative to a no-intervention control condition. Tables X-X show the results of these regression analyses for Experiment 3a, 3b, and the two studies combined. In each analysis, the coefficient representing the interaction between phase and condition is positive and credibly differs from zero, indicating that the vaccine toxins intervention positively impacted vaccination intentions.
...
make_table_df() %>% 
  mutate(
    Term = case_when(
      ...
      Term == "item_sd(Intercept)" ~ "$\\sigma_{item}$",
      Term == "subj_sd(Intercept)" ~ "$\\sigma_{subj}$",
      TRUE ~ gsub("(intervention|posttest)","", Term)
    ),
...
```{r echo=F, warning=F, message=F, fig.width=8, fig.height=4, fig.cap="Belief changes following the intervention in Study 4. Left, mean difference scores from pretest to posttest for the 14 belief scales among participants who did and did not report exposure to news of the 2019 Measles outbreak in New York. Right, mean observed and model predicted change scores for those who were exposed to news of the outbreak. In both plots, error bars represent 95\\% confidence intervals."}
s4_diff_plot <- combined %>% 
  ...
  ggplot(aes(x=reorder(scale, mean), y = mean, ymin =ll95, ymax=ul95, color=vacc_news, shape=vacc_news)) +
  ...
  scale_color_manual(values=c("#0067b1","grey")) +
  theme_bw(base_size=11) +
  theme(
    axis.text.x = element_text(angle=45, hjust=1),
    panel.grid = element_blank(),
  ...
  labs(title="A", y = "Mean difference", x = "Belief scale", color="News Exposure", shape="News Exposure")
...
results <- list(
  s1 = list(demo = s1_meta),
  s2 = list(demo = s2_meta, reg = fit_s2_vaccIntent),
  s3 = list(demo = s3_meta, reg = fit_s3_vaccIntent),
  s4 = list(demo = s4_meta, reg = fit_s4_vaccIntent)
)
