...
authors: "Derek Powell, Kara Weisman, and Ellen M. Markman"
output:
  pdf_document: 
...
```{r, echo=F, message=F, warning=F, fig.width=4, fig.height=3, fig.align='center', fig.cap="Example DAG representing some variables related to wildfires"}
...
grViz(
  ...
  export_svg() %>%
  xml2::read_xml() %>%
  xml2::write_xml("../local/graph.svg")

knitr::include_graphics("../local/graph.svg")
```
...
$$y = 1 \big[1-(1-w_0)\prod_i(1-w_i)^{x_i}\big]\prod_j(1-w_j)^{x_j}$$ 
...
```{r cross_validation, echo=FALSE, fig.height=4, fig.width=6, fig.cap="Cross-validation performance for diffrent structure learning algorithms on the training data from Study 1."}
...
# possibly a bootstrap replicate was not full rank / produced huge colinearity?
...
```{r validation_plot, echo=FALSE, fig.height=6, fig.width=10, fig.align="center", fig.cap="Model predictions versus observed values for test data from Study 1."}
...
```{r, echo=FALSE, message=F, fig.height=8, fig.width=8, fig.align='center', fig.cap = "Correlations among 14 the belief scales measured in Study 1. Correlation matrix is arranged by hierarhical clustering algorithm to highlight connections among beliefs."}
...
ggplot(temp_cor_df,
  ...
  geom_text(aes(label = format(round(cor, 2), digits = 2)), size = 3.5) +
...
# s2_demo needs recoding / renaming
# s3_demo good 
# s4_demo same as s1
