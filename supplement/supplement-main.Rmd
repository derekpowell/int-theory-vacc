...
anti_join(as.data.frame(undirected.arcs(main_cpdag)), (ruled_out %>% bind_rows(ruled_out %>% rename(to="from", from="to")) ), by = c("from", "to"))
...
```{r}
boot_df <- boot_res %>% filter(strength>.2, direction>0)
```
...
z$edges_df$label <- round(edge_df_mod$value,2)
# z$edges_df$color <- ifelse(edge_df_mod$value > 0, "blue","red")
z$edges_df$color <- map_to_brewer(edge_df_mod$value, option="RdBu")
...
## it's must easier just to export them both and then combine with a graphics app
...
$$ ln\big(O(h|d)\big) = ln \big(O(h)\big) + ln\Bigg(\frac{P(d|h=1)}{P(d|h=0)}\Bigg) $$
...
make_table <- function(table_df, caption=NULL){
  ...
  kableExtra::kbl(table_df, digits = 2, booktabs = T, caption=caption) %>% 
...
```{r echo=F, message=F, fig.align='center', fig.height=3, fig.width=3}
...
Due to the similarity of their designs, we combined data from studies 3a and 3b for analysis. As in study 2, we conducted a mixed-effects Bayesian ordinal regression to assess the effects of the vaccine toxins intervention on participants’ Vaccination Intentions scores. The vaccine toxins intervention improved participant’s vaccination intention scores relative to the control conditions. However, it is worth noting that reliable changes were also observed from pretest to posttest in the control conditions, suggesting that these attitudes were also being influenced by other factors during the course of the study.
...
make_table("Bayesian regression model coefficients for model of Vaccine Intentions in Experiment 3.")
```
...
```{r, echo=F, fig.width=4, fig.height=4, fig.align="center", fig.cap="Plots of belief changes following the intervention in Experiment 3. Left, mean difference scores from pretest to posttest for the 14 belief scales measured in Experiment 3. Mean observed and predicted change scores for Experiment 3. Error bars in both plots represent 95\\% confidence intervals. "}
