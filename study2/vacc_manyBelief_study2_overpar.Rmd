...
This is interesting and a great setup, but I'm having a bit of trouble interpreting 3-4 way interactions ...
...
fit <- brm(
  vaccIntent ~ phase * condition * overpar * medSkept * diseaseSevere + (1|workerId),
  prior = set_prior("normal(0,.5)", class = "b"),
  family = Beta(),
  data = d_scored_wide %>% 
    mutate(vaccIntent = rescale_beta(vaccIntent, 
                                     min(d_scored_wide$vaccIntent), 
                                     max(d_scored_wide$vaccIntent)
                                     )),
...
I don't think any of these interaction terms are coming out "significant." Certainly all the phasepost:conditionDR involved ones are not. Closest looks like phasepost:conditionDR:overpar:medskept.

```{r}
fit_pre <- brm(
  vaccIntent ~ overpar * medSkept * diseaseSevere + (1|workerId),
  prior = set_prior("normal(0,.5)", class = "b"),
  family = Beta(),
  data = d_bn %>% 
    mutate(vaccIntent = rescale_beta(vaccIntent, -3, 3), workerId = 1:nrow(d_bn)),
  control = list(adapt_delta = .90, max_treedepth = 15),
  chains = 2,
  cores = 2
)
