...
authornote: |
  Supplementary Materials, including all data and analysis code for this project are available at https://osf.io/sdfpj/. Preregistration information for Studies 3a and 3b is available at https://osf.io/uemq4. Earlier modeling and analyses of data from Study 1 were presented at the 40th Annual Conference of the Cognitive Science Society. A six-page paper also appeared in the proceedings [@powell.etal2018]. 
...
1. When two beliefs are related by an intuitive theory, evidence directly affecting one of those beliefs will also affect the other in accordance with their relationship in the intuitive theory (e.g. evidence for B will increase credence in A, and vice versa). This is a clear and direct consequence of the laws of probability and Bayes' rule. 
2. Where beliefs about two states of affairs, A and B, are related through an intuitive theory (e.g., A causes B), those beliefs will be systematically correlated across individuals in proportion to the average conditional probabilities (e.g. people who believe A will also tend to believe B); This can be seen through an application of the law of total probability, which implies a linear relationship between the two beliefs across individuals. 
3. Thus, we can often reasonably expect there will be some systematic relationship between the average change in each belief following observation of some evidence and the correlation among those beliefs across individuals. We provide a more formal explication of these last two points in Supplementary Materials.
...
Figure 1 presents several examples of Bayesian networks. Each of the networks is a _directed acyclic graph_ (DAG), a type of graphical model. Graphical models represent variables as nodes connected by edges. In a DAG, these edges are directed, flowing from parent to child, and are constrained so that they cannot form cycles or loops. In a Bayesian network, these directed edges encode conditional dependencies among variables, so that the conditional probability distribution for a given variable is defined only in terms of its direct parents. 

(ref:bn-explain-caption) Several examples illustrating different features of Bayesian Networks. \newline A) A graphical representation of influence relations: Fire causes smoke. This influence is represented by the conditional dependencies captured in a conditional probability table (CPT). \newline B) A “parent” node can be linked to multiple “children” and a “child” node can be linked to multiple “parents”: A child and its parents together compose a “family”. This graph has three families: {Accidents, lightning, drought, fire}, {fire, smoke}, and {fire, ash}. \newline C) Not all influence relations correspond to causation: we could say a storm _consists_ of rain and lightning (marked as dashed arrows), which in turn causes thunder. Causes can be generative or preventive, e.g. rain prevents drought and fire (red arrows). Some nodes may exert countervailing influences on others: A storm consists of rain, which can prevent fires, but a storm also consists of lightning, which can cause fires. In addition, parents may influence their children independently or interactively. Lightning and accidents could be seen as independent causes of fire—an accident may cause a fire whether or not there is lightning, and vice versa. However, drought conditions interact with both of these other parent nodes, making them more likely to produce a fire. In the present work, we will consider only the role of independent influence relations.


```{r, out.width='100%', fig.align='center', fig.cap = "(ref:bn-explain-caption)"}
knitr::include_graphics("fig-bn-examples.png")
```


In Figure 1a, the edge from $fire \rightarrow smoke$ indicates that the probability of smoke is increased by fire. So, conditional on fire, smoke becomes more likely. The Bayesian network captures how conditioning on any one node affects the probability of all the other nodes in the network. The directed edges in a Bayesian network capture _asymmetric_ relationships: different edge directions imply different conditional dependencies and independencies. A criterion called “d-separation” provides a formal means for deciding which nodes in a network conditionally depend on one another [@pearl1988]. To illustrate, consider some relationships depicted in Figure 1c. Intuitively, it should be clear that observing (conditioning on) lightning will affect the probability of fire and all of its descendents. Indeed, these nodes are “d-connected” in the network. But, observing (conditioning on) lightning should not affect the probability of accidents (these nodes are d-separated in the network). 

It can be tempting to interpret the edges in a Bayesian network as representing causal relationships. Strictly speaking however, the Bayesian network is simply an encoding of the probabilistic relationships among variables. These dependencies can be interpreted most generally as representing “influence relations” [@williamson2001]. As modelers, we might sometimes give these influence relationships more specific interpretations: causal relationships and logical implications [@williamson2001] are two prominent sorts of influence relations. In other cases, influence relations might be interpreted as representing “constituency” (e.g. see Figure 1c) or an entity's having a particular property (e.g. an edge from "being an apple" to "being red" might represent that redness is a common property of apples). However, in all these cases these interpretations are external to the Bayesian network model itself, where the edges signify only probabilistic relationships.

Bayesian networks like those depicted in Figure 1 can be made to capture each of the qualitative features of intuitive theories laid out above. First, a Bayesian network captures people’s reasoning about these states of affairs given observations: observing smoke would lead someone to infer that there is likely to be a fire, and observing fire would lead someone to predict smoke will soon follow (Feature 1). Using the conditional probability values in Figure 1a, observing smoke would shift someone with a prior probability of fire $p(fire) = .2$ to a posterior probability of fire $p(fire|smoke) = .83$. And for this same person, observing fire would shift their credence in smoke from $p(smoke) = .238$ to $p(smoke|fire) = .99$. 

This model can also capture how these beliefs might covary across individuals: if two people hold different degrees of belief in _fire_, but share the same understanding of the relationship between _fire_ and _smoke_, this model specifies the degree of belief each should have in _smoke_ (Feature 2). If Alice holds that $p(fire) = .4$ and Bob holds that $p(fire) = .1$, then Alice should belief $p(smoke) = (.99)(.4) + (.05)(.6) = .426$ and Bob should believe $p(smoke) = (.99)(.1) + (.05)(.9) = .144$.

We can leverage both of these facts to derive a model of people’s intuitive theory relating these affairs based on the correlations among these beliefs. Because the conditional probabilities of _smoke_ given _fire_ and $\neg fire$ crucially determine both the change in beliefs given observations and the correlation of beliefs across individuals, average changes in beliefs following evidence will generally be related to the initial correlation of those beliefs across individuals (Feature 3; see Supplementary Materials for further explanation). 

Formulating these features in a formal model, rather than a verbal theory, allows us to specify not just the presence but the quantitative strength of these relations, and to simulate the cumulative influences of many factors in more complex networks (for example, incorporating beliefs about the frequency of wildfires, recent weather conditions, etc.). 

Other network-based approaches have also been applied to model systematic relationships among beliefs and attitudes, such as neural network models [e.g. see @holyoak.powell2016; @holyoak.simon1999; @simon.etal2004; @thagard1989; @spellman.etal1993]. In these models, beliefs or attitudes are represented by nodes (neurons) connected by undirected or bi-directed edges---in contrast to the directed edges used in Bayesian Networks. One application of neural-network models has been the modeling of “coherence-shifts”: patterns of belief revision wherein the beliefs or attitudes that should putatively serve as inputs to an evaluation are themselves modified during the process of evaluation. However, the directed edges of Bayesian networks are similarly capable of capturing these coherent reasoning dynamics: When conditioning on a node within a network, information flows in both directions along the edges. To illustrate, consider how observing smoke will lead a reasoner to infer that there is a fire, just as readily as observing fire will lead them to predict there will be smoke.[^1] 

[^1]: With this in mind, Bayesian Networks also offer potential models of coherence-based reasoning effects based on rational belief updating mechanisms [@jern.etal2014].
...
This parameterization is a substantial simplification compared with the full flexibility possible within a Bayesian network. A cognitive model composed of these types of relations is capable of capturing a causal system of non-interactive generative and preventive causes. But it bears noting that this simplification comes with limitations. For instance, returning to the example network in Figure 1c, this parameterization would not permit capturing the relationships between dryness, lightning, and fire. Dryness is an enabling condition that both increases the probability of fire but also increases the probability that lightning would cause a fire. So the influences of these variables are not independent and would need a more complex parameterization to be properly represented.

Finally, having transformed participants’ scores on each belief scale to fall between 0 and 1, we estimated the parameters of this model as a Beta regression using maximum likelihood methods (see Supplementary Materials for details).

#### A cognitive model underlying the data model. 

We aimed to represent not only the relationships among variables in the data but the *cognitive model* that gave rise to these observed relationships, i.e., the relationships among beliefs in the mind. We actually estimated two models sharing the same network structure as depicted in Figure 2: First, we estimated a data model assuming that participants possess a shared mental model from which they generate their responses to the belief scales. In this model, the values of $p(y=1)$ and $x$ in the equation above corresponded to participants’ continuous responses to the belief scales (e.g. a participant's score for the *vaccine danger* scale). Then, from the parameters estimated in the data model we created a cognitive model of participants’ reasoning, where the $x$ are binary, indicating whether that state of affairs obtains (e.g., whether “vaccines are dangerous”). 

We used this cognitive model to simulate participants' degrees of beliefs given other beliefs, as well as how beliefs would change following exposure to evidence.
...
To conduct an efficient search through the super-exponential space of graphs, we tested several learning algorithms implemented in the “bnlearn” R package [@scutari2010] and evaluated their performance through cross validation. To avoid overfitting, we used an 80% split of the data (*n* = 904) for training, and reserved the remaining 20% for evaluating model performance. We compared performance of different structure learning algorithms using k-fold cross validation within the training split. Graphs were scored by Bayesian Information Criteria, an asymptotic estimate of the likelihood of the data under a given graph structure. Based on these results, we chose to apply a score-based "hill-climbing"  algorithm using our generative model constraints (see Supplementary Materials for details). Applying these techniques resulted in an estimate of the network shown in Figure 2. 

```{r fig1-maingraph, fig.align='center', out.width="90%", fig.cap="Bayesian network cognitive model resulting from application of hill-climbing algorithm with our generative-model constraints applied to data in Study 1 training split (n = 904). The network has 29 edges connecting these 14 variables. Edge polarity and weights are indicated by color, with red hues indicating negative or preventive influences, and blue hues indicating positive or generating influences. The direction of the edge was not determinable from the data and modeling constraints for two edges: $\\text{Naturalism} \\rightarrow \\text{Holistic Balance}$ and $\\text{Medical Skepticism} \\rightarrow \\text{Parental Expertise}$; see Supplementary Materials."}
...
We evaluated the resulting cognitive model’s performance by testing its accuracy in predicting responses in the 20% testing split (*n* = 226). We generated predictions for held-out participants’ responses for each variable by conditioning the cognitive model network on the remaining 13 observed variables. Figure 3 compares the model’s predictions with participants’ actual responses. Collapsing across all variables, the average correlation between predicted and observed responses was *r* = `r round(avg_corr, 3)`, accounting for `r round(100*avg_corr^2, 0)`% of the variance in observed responses.

Our goal in developing this cognitive model was to create a psychologically plausible and intelligible model of the intuitive theory surrounding vaccination decisions. However, it is also reasonable to ask whether other less-constrained models might be more predictive. To address this, we compared the Bayesian network model’s predictions to a suite of 14 naive “maximal” regression models, each using values on all 13 other belief scales as predictors for each belief. The model presented in Figure 2---which we take to be simpler, more psychologically plausible, and more intelligible---performed as strongly as these "maximal" regression models (see Supplementary Materials).

```{r fig2-validation, fig.align='center', out.width="100%", fig.cap="Observed responses and cognitive model predictions for each belief among participants in the Study 1 testing split. Correlations between observed and predicted values ranged from .380 to .898 across the different belief scales. In general, the model showed greater predictive accuracy for more central beliefs (e.g., \\emph{vaccine danger}) than for more peripheral beliefs (e.g., \\emph{parental protectiveness}; see Supplementary Materials for details)."}
knitr::include_graphics("../local/valid_plt.png")
```

#### What does this cognitive model reveal about the intuitive theory underlying vaccination decisions? 
...
It is worth pointing out that not all of the relationships represented in the model should be interpreted as _causal_. Due to Bayesian networks’ tight connections with structural causal networks, it can be tempting to give the edges this interpretation, but in some cases these relationships are better thought of as probabilistic dependencies or “influence” relations [@williamson2001]. For instance, the edge running from vaccine danger to toxic additives in vaccines clearly does not represent a causal influence: vaccines being dangerous does not cause them to contain toxins. Instead, a more plausible interpretation of this edge is that it reflects something like a set membership relation: that toxins are one type of danger that might be present in vaccines.

In other cases, our data and explicit constraints do not perfectly identify the directions of the edges. This is the case for two edges (Naturalism $\rightarrow$  Holistic Balance and Medical Skepticism $\rightarrow$ Parental Expertise; see Supplementary Materials for details). In both cases, we do not feel it is intuitive which direction the edge should flow in, nor even that it should be directed at all. Such a counterintuitive result might be taken as indicating a limitation of Bayesian network models, which are composed solely of directed, acyclic relationships. We note that prior work has successfully modeled coherent belief revision by leveraging models with undirected or bidirected relations [e.g. @holyoak.simon1999; @simon.etal2004; @thagard1989; @spellman.etal1993]. Alternatively, we might instead hypothesize there is an additional unmeasured variable that connects these beliefs that could more intelligibly account for their association within the context of a directed network. For instance, a more abstract epistemic belief might influence both medical skepticism and parental expertise beliefs. To speculate, such an abstract belief might be in one's own expertise or in how we come to have knowledge (e.g. through direct experience versus science).

More generally, as they operate solely on correlations among variables, structure learning algorithms cannot be guaranteed to correctly identify causal directionality. Looking forward to future applications of these methods, researchers might seek other ways to resolve ambiguities left by structure learning algorithms. The simplest approach would be to set edges manually based on top-down theorizing, or even to set stronger constraints in advance of fitting models. Another potential future direction would be to probe people's mental causal models more directly, either by asking them directly what they think about the causal relationships among variables or by asking them to reason through hypothetical interventions on variables. These more direct responses might then be used to constrain or augment the structure learning algorithm's outputs.
...
Only a handful of educational interventions have been shown to improve attitudes toward childhood vaccination. One of the successful educational interventions in this domain was tested by Horne and colleagues [-@horne.etal2015], who found that emphasizing the dangers of preventable diseases like mumps, measles, and rubella improved participants’ attitudes toward vaccines. Their intervention included descriptions of the symptoms of mumps, measles, and rubella; vivid photos of children suffering from these diseases; and a brief story from the perspective of a parent whose child was hospitalized with measles (see Supplementary Materials). This “disease risk” intervention improved attitudes toward vaccines compared to an irrelevant control essay and compared to an intervention aimed at debunking concerns about vaccines causing autism. In Study 2, we aimed to replicate this success and leverage the cognitive model developed in Study 1 to gain insight into how and why such an intervention successfully induces  belief revision in this domain.

In Study 2, we used the cognitive model established in Study 1 to predict how Horne and colleagues’ [-@horne.etal2015] intervention would affect the beliefs in this network, including people’s intentions to vaccinate their children. Participants completed the 14 belief scales from Study 1, then returned two days later to complete either the disease risk intervention or a no-intervention control, followed by the same 14 belief scales.
...
Participants (*n* = `r results$s2$demo$n_complete` passed attention checks in total) in the intervention conditions were presented with a version of Horne and colleagues'  “disease risk” intervention, modified to present subparts in a fixed (rather than randomized) order, with slight changes to wording to improve clarity, and with a final statement summarizing the take-home message of the intervention; see Supplementary Materials. They then completed the same 14 belief scales and demographic questions that they had completed in Phase 1. 
...
Where *x* was a binary variable representing the presence (1) or absence (0) of evidence at posttest, and participants’ prior and posterior (pretest and posttest) credences were transformed into log odds. The variables $ER_0$ and $ER_1$ represent measures of the evidence seen by participants in the control and intervention conditions. These are defined as:

\begin{equation}
ER_i = \ln\frac{(P(d_i|h=1))}{(P(d_i|h=0))}
\end{equation}

In equation 3, $ER_0$ represents whatever background "evidence" participants would see between phases outside  the experimental context, that would be shared by participants in both conditions. $ER_1$ represents the evidence provided by the intervention information, which is assumed to be independent from $ER_0$. These log evidence ratios were both estimated via regression. Then, we constructed a conditional probability table consistent with $ER_1$ and augmented the cognitive model with this evidence node (see Supplementary Materials for details). 

Using this augmented model, we are able to “condition” our cognitive model on this evidence node, and thereby predict how observing evidence will affect all of the beliefs in the network.  As shown in Figure 4, panel B, this simulation is a reasonable approximation of what we observed in Study 2: The model’s predictions correlate strongly with observed changes (*r* = `r results$s2$modcorr`). 

Given that our cognitive model is parameterized via noisy-logical functions wherein each parent independently influences each child node, it should be noted that the marginal predicted influence of each belief is closely related to its first-order correlation with disease severity. This is in keeping with the qualitative features of intuitive theories we laid out in the introduction, and we should often expect this to hold true on average for any plausible model of an intuitive theory. However, future models with more complex or nuanced parameterizations could potentially make additional predictions not evident from first-order correlations, such as individualized predictions for how individuals or groups would respond to evidence.
...
One advantage of having developed a formal, computational representation of a network of beliefs supporting vaccination decisions is that it can guide our search for effective interventions, allowing us to compare how interventions targeting each of these other beliefs might potentially change people’s vaccination intentions (see Figure 5). Countering myths about toxic additives is one approach the cognitive model developed in Study 1 predicts should be effective.

```{r fig4-predimpacts, fig.align='center',  out.width="50%", fig.cap="Predicted change in vaccination intentions based on cognitive model following an equivalent hypothetical change in each of the other 13 beliefs."}
...
As they shared the same design, we combined data from Studies 3a and 3b for analysis (but see Supplementary Materials for separate analyses by study). As in Study 2, we conducted a mixed-effects Bayesian ordinal regression to assess the effects of the vaccine ingredients intervention on participants’ *vaccination intentions* scores. 

As in Study 2, Study 3a and 3b used a difference-in-difference design. Thus the effect of the vaccine intervention is measured by testing the interaction between condition and phase. The vaccine ingredients intervention improved participants' vaccination intention scores relative to the control conditions (interaction effect: $b$ = `r round(s3_reg$fixed[9,1],3)`, 95% CI: [`r round(s3_reg$fixed[9,3], 3)`, `r round(s3_reg$fixed[9,4], 3)`]); see Figure 6, panel A. It is worth noting that reliable changes were also observed from pretest to posttest in the control conditions (phase effect: $b$ = `r round(s3_reg$fixed[7,1],3)`, 95% CI: [`r round(s3_reg$fixed[7,3], 3)`, `r round(s3_reg$fixed[7,4], 3)`]), suggesting that these attitudes were also being influenced by other factors during the course of the study. Given that these studies were randomized controlled trial designs, these findings do not impair our ability to conclude that the intervention was causally efficacious in affecting vaccine intentions. It is important to emphasize that the test of the intervention's effectiveness is a test of an interaction---an interaction demonstrating that the improvements in intentions in the intervention condition were greater than those in the control condition. 

#### Modeling belief revision

To model the effect of the vaccine ingredients intervention, we followed the same procedure as in Study 2, but this time augmented the cognitive model with a node representing the vaccine ingredients intervention as a child of the vaccine toxins belief node (for details, see Supplementary Materials). This allowed us to predict how other beliefs in the network would change in light of the observed changes in participant’s beliefs about whether vaccines contain dangerous toxins; see Figure 6, panel B.

Collapsing across these two studies, Figure 6 shows the change in participants’ (*n* = `r results$s3$demo$n_complete`) attitudes across all 14 belief scales from pretest to posttest. Most importantly, the vaccine ingredients intervention significantly improved participants’ vaccination intentions compared to the no-intervention control condition. Changes were also evident across many of the 14 beliefs, with the largest changes being a reduction in vaccine toxicity beliefs. 

```{r fig5-s3, fig.align='center',  out.width="100%", fig.cap="Belief changes following the intervention in studies 3a and 3b. Left, mean difference scores from pretest to posttest for the 14 belief scales across experimental conditions. Scales are arranged in order of their predicted changes according to the cognitive model. Right, mean observed and  predicted change scores from the cognitive model for the disease risk intervention condition. In both plots, changes were calculated after combining data from both studies and transforming responses to lie between 0 and 1 and error bars represent 95\\% confidence intervals."}
knitr::include_graphics("../local/s3_plt.png")
```

As in Study 2, we used our model to predict how the 13 other beliefs in the intuitive theory should be expected to change given the average magnitude of the change observed in participants’ vaccine toxicity beliefs following the intervention. As shown in Figure 6, the model’s predictions are significantly related to the observed changes, but the correlations are relatively weaker (*r* = `r results$s3$modcorr`) than those observed in our other studies. 
...
```{r fig6-s4, fig.align='center',  out.width="100%", fig.cap="Participants’ belief changes from participation in Study 1 to Study 4. Left, mean difference scores comparing beliefs reported in August-September 2017 and May 2019 for the 14 belief scales, across participants who did and did not report exposure to news of the ongoing New York measles outbreak. Scales are arranged in order of their predicted changes according to the cognitive model. Right, mean observed and  predicted change scores from the cognitive model for those participants who did report exposure to this news. Belief changes were modeled assuming that news of the outbreak directly impacted participant's vaccine intentions, the most central of the 14 beliefs, in order to evaluate the overall coherence of these belief changes. In both plots, changes were calculated after transforming responses to lie between 0 and 1 and error bars represent 95\\% confidence intervals."}
...
Figure 7, panel A, shows the average change in participants’ beliefs following the outbreaks of 2019. Among participants who were exposed to relevant news covered, we observed changes in a number of beliefs, with notable increases in intentions to vaccinate, perceptions of vaccine efficacy, and perceptions of disease severity, as well as notable decreases in concern about vaccine danger. In contrast, most beliefs remained unchanged for those participants who did not report exposure to media, although there were modest changes in their beliefs about disease severity and disease rarity. These outbreaks appear to have significantly impacted people’s beliefs surrounding vaccination.

```{r, include=F}
s4_reg <- summary(results$s4$reg)
```

Mirroring our analyses of interventions, we regressed responses to the *vaccine intentions* scale items on predictors for phase (August-September 2017 vs. May 2019, dummy-coded), news exposure (dummy-coded with non-exposure as the baseline), and an interaction between phase and news exposure onto participants’ scores, using a Bayesian mixed-effects ordinal regression including random intercepts by subject and item. To assess the effects of recent news exposure on vaccine intentions, we examined the interaction between phase and news exposure. Participants’ vaccine intentions were increased by recent news exposure compared to their responses 18 months earlier (interaction effect: $b$ = `r round(s4_reg$fixed[9,1],3)`, 95% CI: [`r round(s4_reg$fixed[9,3], 3)`, `r round(s4_reg$fixed[9,4], 3)`]); see Figure 7, panel A. 

#### Modeling belief revision

Were changes in participants' beliefs following exposure to news of the measles outbreak coherent with respect to the intuitive theory specified by the cognitive model developed in Study 1? Unlike in the intervention studies, where a specific belief was quite precisely targeted by the intervention, it is likely that many beliefs were addressed by media coverage of these outbreaks. Given that we cannot measure the exact information that participants were exposed to, it is not feasible to model the precise locus of its impacts. However, we can leverage our model to assess whether the changes in participants' beliefs were *coherent*. 

Throughout our prior studies, we have focused primarily on *vaccine intentions* as the most crucial and proximal belief for determining vaccination decisions. So, we sought to examine whether the changes across all the measured beliefs were coherent with respect to the changes in vaccination intensions. Using our model, we generated predictions for how the 13 other beliefs in the intuitive theory should be expected to change given the average magnitude of the change observed in vaccine intentions. To accomplish this, we followed the same procedure as in previous studies, but here augmented the model with a node representing media exposure as a child of the *vaccine intentions* node. This provides an evaluation of coherence similar to prior work on coherence-based reasoning, where the coherence of belief revision is evaluated with respect to some emerging decision, evaluation, or change in one specific belief or attitude [e.g. @holyoak.simon1999; @simon.etal2004; @spellman.etal1993].

As shown in Figure 7, panel B, the changes in participants’ beliefs are very similar to the model’s predictions (*r* = `r results$s4$modcorr`), suggesting that the changes in their beliefs were highly coherent with respect to their intuitive theories of the domain. Modeling these beliefs as united by an intuitive theory not only captures how these beliefs are revised immediately in light of evidence, but also how they are changed by real-world events.  
...
We tested two educational interventions: one emphasizing the dangers of childhood diseases and another debunking concerns about toxins in vaccines. Both were effective in increasing people’s intentions to vaccinate. How might these educational interventions affect vaccine uptake if deployed in the real world? We are somewhat cautious in our optimism: In conventional terms of standardized effect sizes, both intervention effects would be considered quite modest and their impacts on actual future decisions remain unknown. Still, we nevertheless see both of these interventions as promising foundations for real-world educational efforts. First, as they are quite brief, these interventions present a very low cost for delivery. In addition, their brevity supports opportunities for deploying them with strategic timing and from trusted sources. For instance, these arguments could be deployed in the context of visits with family physicians, including immediately before vaccination decisions are to be made. Delivering these messages at the point-of-decision could maximize their impacts. And, we predict their efficacy would be improved if they were delivered by a trusted family physician, rather than coming from a relatively anonymous online survey. 

We credit our development of a quantitative model of the intuitive theory surrounding vaccination decisions as supporting our work as interventionists. By providing a graphical representation of the structure of this intuitive theory, this approach helped us to understand relationships between beliefs that were puzzling when considered only as correlations (e.g., the lack of correlation between beliefs about the infant immune system and vaccine intentions), and allowed us to “spot” opportunities to develop novel interventions. One exciting future direction for these and other brief interventions would be to tailor these interventions to an individual’s beliefs: Models with richer parameterizations could simulate belief change for an individual given a certain set of prior beliefs, paving the way for determining which specific intervention or combination of interventions would have the most impact for a particular decision-maker.
...
First, in the spirit of Horne and colleagues’ [-@horne.etal2015] disease risk intervention, we suspect that education focused on how severely ill many people become and the prevalence of long-term health problems following even mild cases of COVID-19 might encourage people to accept coronavirus vaccines. Indeed, controlled intervention studies have shown that messages focused on personal risk increase vaccination intentions [@motta.etal2021]. However, the severity of COVID-19 is highly variable across individuals and across variants of the virus, potentially rendering the formation, updating, and use of beliefs about severity much more nuanced in the case of COVID-19 than in the case of diseases like measles. 

Our findings also suggest that beliefs about vaccine efficacy are central to determining vaccination intentions. The high prevalence and mutation rate of COVID-19 makes the risk of infection substantial even for the vaccinated, although these vaccines remain highly effective at preventing severe illness and death. The rising prevalence of breakthrough infections and the need for boosters relatively soon after initial doses are likely to lead people to doubt the effectiveness of vaccines---but we see promise in educational interventions that clarify the many ways in which a vaccine can be “effective” (e.g., by preventing infection vs. by preventing severe illness, preventing death, or lowering transmissibility).

Looking forward, living through the COVID-19 pandemic is likely to have deepened the public’s understanding of infectious diseases and public health. For example, the concept of “herd immunity” has likely never had greater public visibility  and appreciation than now. This understanding seems like a prime target for future interventions aimed at increasing the uptake of vaccines for both COVID and other diseases.

Finally, like all pandemics, the COVID-19 pandemic is inescapably social. In the U.S. and abroad, COVID-19 vaccines and mitigation measures have become deeply politicized. According to Gallup polling in the U.S., COVID-19 vaccination rates have at times been nearly twice as high among Democrats as among Republicans [@gallup2021]. Messages from political elites often imply concrete beliefs about the world that could be incorporated into a model of an intuitive theory: for instance, that COVID-19 is "like the flu" or that infection can be treated with off-label pharmaceuticals—or, conversely, that COVID-19 vaccines are safe and effective. Still, these affiliations are also associated with social commitments and personal values that may behave differently than the beliefs examined here. Whereas beliefs about “matters of fact” might be driven by an underlying intuitive theory and thereby sensitive to evidence, these sorts of identity-constituting “beliefs” may operate by different principles [@vanleeuwen2014], such as being driven primarily by people’s affinity toward their social groups rather than by concrete external evidence [@kahan2016].

More broadly, people from different social groups have different experiences and knowledge that are likely to shape their intuitive theories of vaccination and the medical establishment in important ways. For example, White vs. Black Americans are likely to have different experiences with medical racism. Future work should explore how differences in people’s wider intuitive theories might explain vaccination behaviors, as well as how a more nuanced understanding of these theories might support messages tailored to different individuals and communities.

## Broader implications and applications

The current studies add to a growing body of work demonstrating how leveraging intuitive theories can spur belief revision and behavior change in the domain of physical health [cf. @weisman.markman2017]. It also introduces a new approach to modeling and understanding intuitive theories more broadly. In prior work, researchers focused on identifying a small set of beliefs or concepts that were core to the intuitive theory and specific attitudinal or behavioral outcomes.  For example, in previous work on vaccination attitudes, researchers focused on how people balanced perceptions of the risk of vaccine side effects and the risk of childhood diseases to inform their intentions to vaccinate their children [@horne.etal2015]. The approach we introduced here expands the set of beliefs considered beyond a small core to a wider set of auxiliary beliefs that may influence decisions. This is made feasible by leveraging a bottom-up approach that aims to discover the network structure connecting those wide-ranging beliefs.

Here we speculate about how this kind of approach might be applied to the topics of climate change and racial justice: complex domains in which variability in people’s beliefs leads to serious societal consequences, and in which widespread misconceptions demand intervention. In both cases, we hypothesize that people’s behaviors are shaped by a wide-ranging network of interconnected beliefs that might be better understand through approaches similar to those developed here. 

Consider first the domain of climate change: Without swift and drastic efforts to reduce CO2 emissions, global climate change threatens to increase extreme weather events, disrupt agriculture, destroy ecosystems, and displace millions of people over the next few decades. Climate change is a complex geophysical, economic, political, and social phenomenon. Many changes, both individual and structural, are likely needed to effectively reduce CO2 emissions and mitigate the worst effects of climate change. Our work suggests that efforts to understand and encourage any of these behaviors might be best-served by seeking to understand the broader web of beliefs that sustain and support them. 

Small models of beliefs related to climate change with Bayesian Networks have already yielded some promising results. Cook and Lewandowsky (2016) developed a model of how a core free-market "worldview" affects beliefs and belief revision about whether human-caused climate change is really happening. Schotsch and Powell (2022) explored people's causal mental models of how CO2 emissions cause global warming and how warming in turn causes different outcomes such as rising sea levels and extreme weather. Elaborating on these small models to discover and model a wider network of related beliefs has great potential for crafting effective interventions in this domain.

We might ask: How do people understand the science connecting CO2 emissions to warming and warming to other negative consequences and do they trust this science? How do people weigh the efficacy of their own individual behaviors versus government regulations? How much do they trust regulators or specific regulations to be effective and how concerned are they about their economic side-effects? How do they expect climate-driven extreme weather, crop losses, economic disruptions, and mass migration will affect them personally? What obligations do they feel toward future generations or even their own children for the kind of world they leave behind? What value do they place on preserving the natural world and its rich ecosystems and biodiversity?

The complexity of this domain will likely present a challenge to cataloging even a wide auxiliary set of beliefs that could fully circumscribe it. Thus the goal need not be to identify an authoritative and all-encompassing "intuitive theory" of climate change.  Rather, interventionists might be better served by developing more constrained models of intuitive theories that bear directly on a specific target behavior of interest.

Intuitive theories may also inform people’s reasoning about social and political issues. To highlight one important example, following the police killing of George Floyd in 2020, protests for racial and social justice organized by groups like Black Lives Matter spread across the U.S. and elsewhere. Across different people and groups of people these events spurred a wide range of reactions: Some called for police departments to be defunded, others for police funding to be increased. Some sought to educate themselves by purchasing anti-racist books, others began pressuring school boards to eliminate purported elements of “critical race theory” from K-12 curricula.  Some were freshly or further awakened to and disquieted by racial injustice in American society, others were persuaded by the white supremacist “great replacement” conspiracy theory. 

One way to try to understand these vastly different reactions is to imagine that they were shaped by large, complex networks of beliefs about the nature of race; about American history and politics; about policing; about justice, equity, and other moral principles; and so on. The approach outlined in the current studies could provide a useful tool for navigating this complexity, yielding insights into the interrelations among these beliefs. 

To draw a few examples from the vast literature on American racism: How does understanding of the historical impacts of slavery, reconstruction, the post reconstruction era and the civil rights era relate to beliefs about the racial inequalities that persist in the present day? How does this historical awareness in turn relate to whether people conceptualize racism as residing in beliefs, attitudes, and behaviors of individuals or as embedded in societal structures, institutions, and practices? How do construals of race as a biological trait versus as a social construct impact thoughts on these issues? And how does the conceptualization of racism as an individual or structural problem encourage people to address racism actively or passively ignore it? It is our hope that mapping out connections among these and other beliefs could reveal new avenues for correcting people’s misconceptions and promoting mutual understanding that could spur anti-racist actions and social change.

## Conclusions

Not all human reasoning need be governed by intuitive theories, but intuitive theories are likely to be crucial wherever people must integrate information to evaluate, explain, or make decisions. Intuitive theories are powerful cognitive tools, for better and for worse. Coherent networks of beliefs allow people to understand, predict, and navigate the world, but they can also sustain misconceptions and lead to suboptimal---even dangerous---decisions and behaviors. The present work on understanding vaccine hesitancy suggests a general theoretical approach to understanding belief revision more broadly: by mapping out a set of beliefs relevant to a given domain, researchers and educators might identify optimal avenues for persuasion and education. Further, by formalizing intuitive theories into quantitative models, our approach supports the use of bottom-up approaches to discover structure among a wider set of auxiliary beliefs and provide more precise tests of theories of belief revision. We hope the studies presented here have shed light on how people might come to form the beliefs and misconceptions that they do, why those misconceptions can be so hard to combat, and how to develop educational interventions that are persuasive and effective in encouraging and sustaining positive behaviors.
