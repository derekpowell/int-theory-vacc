...
cors <- df_preds %>%
  group_by(scale) %>%
  summarise(correlation = cor(predicted, observed))
...
pred_post %>%
  ...
  group_by(scale) %>%
  summarise(posttest = mean(response)) %>%
  left_join(
    (
      d_interv_pre %>%
  gather(scale,response) %>%
  group_by(scale) %>%
  summarise(pretest = mean(response))
    ), by = "scale"
  ) %>%
...
Another thing worth considering is taht this isn't a "model" in any traditional sense. It's just a method of compressing and reconstructing the data. So in the predictions for s1, it's not really "predicting " each variable without being given any data on that specific variable. Instead, it's being given all the variables, then compressing them, then seeing if it can reconstruct them, but the values for, e.g., vaccine intentions are used to create the compressed components that are used in the reconstruction, so it's not really an out-of-sample "prediction" in the same way that the bayesian network (or regression models) are doing.
